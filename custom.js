...
function updateActiveThumbnail() {
  ...
  // Get the overflow-x property from the computed style
  const overflowXProperty = computedStyle.overflowX;
  console.log(overflowXProperty);
  if (overflowXProperty === 'hidden') {
    const thumbnailItemWidth = thumbnailContainer.querySelector('.thumbnail__item').offsetHeight;
    thumbnailContainer.scrollTop = (currentIndex - 6) * thumbnailItemWidth;
  } else {
    const thumbnailItemWidth = thumbnailContainer.querySelector('.thumbnail__item').offsetWidth;
    thumbnailContainer.scrollLeft = (currentIndex - 2) * thumbnailItemWidth;
  }
}
